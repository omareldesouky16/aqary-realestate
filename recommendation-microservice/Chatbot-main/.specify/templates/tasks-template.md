...
- [ ] T010 Implement session ownership checks for all chatbot requests
- [ ] T011 Configure server-side LLM provider access, retry handling, and fallback paths
- [ ] T012 Create deterministic slot resolvers/validators before any search story begins
- [ ] T013 Add privacy-safe rendering and untrusted-content handling foundations
...
- [ ] TXXX [P] [US1] Journey test for authenticated chat, slot resolution, and safe fallback behavior
...
- [ ] TXXX [US1] Verify no LLM-derived value reaches SQL or persisted state without validation
...
- [ ] TXXX Review resolver logs, failed searches, complaint signals, and provider failures
- [ ] TXXX Validate prompt-injection, XSS, phone privacy, and installment redirect demo flows
...
- Session ownership, LLM boundary, and slot resolver foundations before chatbot endpoints
